# #############################################################################
#
#   DEDICATED INBOUND DESIGN — Centralized Inbound VPC with Spoke Applications
#
#   All inbound internet traffic enters through a dedicated inbound VPC
#   containing IGW, GWLB endpoints, and application load balancers. Traffic
#   is inspected by VM-Series firewalls in the security VPC before being
#   forwarded to application workloads in spoke VPCs via Transit Gateway.
#
#   Outbound and east-west traffic inspection remains centralized in the
#   security VPC, same as the combined design.
#
#   Spoke VPCs have NO internet gateways and NO GWLB endpoints — all
#   inbound traffic is funneled through the dedicated inbound VPC.
#   Endpoint subnets are preserved in spokes for future use.
#
...
#
#   - NAT Gateways are deployed in the security VPC to provide internet
...
#               (security VPC) → GWLB endpoint → ALB/NLB (inbound VPC)
#               → spoke VMs via TGW
#
#   - OUTBOUND: Spoke VMs → TGW → security VPC → outbound GWLB endpoint
#               → VM-Series → NAT GW → Internet
#
#   - EAST-WEST: Spoke A → TGW → security VPC → east-west GWLB endpoint
...
#   sed -i 's/<SEC_NET>/10.51/g; s/<INBOUND_NET>/10.50/g; s/<APP1_NET>/10.104/g; s/<APP2_NET>/10.105/g; s/<PANORAMA_NET>/10.255/g' dedicated_inbound_design.tfvars
#
# -----------------------------------------------------------------------------
...
#    [ ] security_vpc cidr   — VPC CIDR sized for your deployment (/22 gives 1024 IPs)
...
#
# 3. NETWORKING — Dedicated Inbound VPC
...
#
# 6. VM-SERIES — Bootstrap & Licensing (choose ONE option)
#
...
name_prefix = "prod-dedicated-" # CHANGE: unique resource name prefix

global_tags = {
  ManagedBy   = "terraform"
  Application = "vm-series-dedicated-inbound"  # CHANGE: application name
...
vpcs = {
  ...
  # ===========================================================================
  # SECURITY VPC — Centralized inspection for all traffic flows
  # ===========================================================================
  security_vpc = {
    name = "security-vpc"
    cidr = "<SEC_NET>.0.0/22"  # CHANGE: security VPC CIDR
    nacls = {
      trusted_path_monitoring = {
        ...
        rules = {
          block_outbound_icmp_1 = {
            ...
            protocol    = "icmp"
            rule_action = "deny"
            cidr_block  = "<SEC_NET>.0.64/28"  # CHANGE: must match private subnet AZ-a
          }
          block_outbound_icmp_2 = {
            ...
            protocol    = "icmp"
            rule_action = "deny"
            cidr_block  = "<SEC_NET>.1.64/28"  # CHANGE: must match private subnet AZ-b
          }
      ...
      vmseries_private = {
        ...
        rules = {
          ...
          geneve = {
            ...
            cidr_blocks = [
              "<SEC_NET>.0.48/28", "<SEC_NET>.1.48/28", "<SEC_NET>.2.48/28"  # CHANGE: must match gwlb subnet CIDRs
            ]
          }
          health_probe = {
    ...
    subnets = {
      # Security VPC uses /28 subnets within a /22 CIDR.
      ...
      # TGW Attach
      "<SEC_NET>.0.0/28" = { az = "us-west-2a", subnet_group = "tgw_attach" }
      "<SEC_NET>.1.0/28" = { az = "us-west-2b", subnet_group = "tgw_attach" }

      # GWLBE Outbound
      "<SEC_NET>.0.16/28" = { az = "us-west-2a", subnet_group = "gwlbe_outbound" }
      "<SEC_NET>.1.16/28" = { az = "us-west-2b", subnet_group = "gwlbe_outbound" }

      # GWLBE East-West
      "<SEC_NET>.0.32/28" = { az = "us-west-2a", subnet_group = "gwlbe_eastwest" }
      "<SEC_NET>.1.32/28" = { az = "us-west-2b", subnet_group = "gwlbe_eastwest" }

      # GWLB (3 AZs)
      "<SEC_NET>.0.48/28" = { az = "us-west-2a", subnet_group = "gwlb" }
      "<SEC_NET>.1.48/28" = { az = "us-west-2b", subnet_group = "gwlb" }
      "<SEC_NET>.2.48/28" = { az = "us-west-2c", subnet_group = "gwlb" }

      # Firewall Private (data plane)
      "<SEC_NET>.0.64/28" = { az = "us-west-2a", subnet_group = "private", nacl = "trusted_path_monitoring" }
      "<SEC_NET>.1.64/28" = { az = "us-west-2b", subnet_group = "private", nacl = "trusted_path_monitoring" }

      # Firewall Mgmt
      "<SEC_NET>.0.80/28" = { az = "us-west-2a", subnet_group = "mgmt" }
      "<SEC_NET>.1.80/28" = { az = "us-west-2b", subnet_group = "mgmt" }

      # Firewall Public
      "<SEC_NET>.0.96/28" = { az = "us-west-2a", subnet_group = "public" }
      "<SEC_NET>.1.96/28" = { az = "us-west-2b", subnet_group = "public" }

      # NAT Gateway
      "<SEC_NET>.0.112/28" = { az = "us-west-2a", subnet_group = "natgw" }
      "<SEC_NET>.1.112/28" = { az = "us-west-2b", subnet_group = "natgw" }
    }
    routes = {
      mgmt_default = {
        vpc           = "security_vpc"
...
natgws = {
  security_natgw = {
    ...
    subnet_group = "natgw"
    nat_gateway_names = {
      "us-west-2a" = "natgw-2a"  # CHANGE: AZs to match your region
      "us-west-2b" = "natgw-2b"  # CHANGE: AZs to match your region
    }
  }
}
...
gwlb_endpoints = {
  # Outbound and east-west endpoints in security VPC (unchanged)
  ...
  # Inbound endpoints in DEDICATED INBOUND VPC (not in spoke VPCs)
  app1_inbound = {
...
# This adds a static route on the TGW for Panorama reachability from the security VPC.
/*
panorama_attachment = {
  tgw_key                      = "tgw"
