# #############################################################################
#
#   COMBINED DESIGN — Single Security VPC with Distributed Inbound
#
#   Single security VPC handles all traffic flows: outbound, east-west, and
#   inbound. Inbound traffic enters each spoke VPC's own IGW and is redirected
#   through distributed GWLB endpoints (deployed in the spoke VPCs) to
#   VM-Series firewalls in the security VPC for inspection.
#
#   Traffic flows:
#
#   - INBOUND:   Internet -> Spoke IGW -> GWLBE (spoke) -> GWLB -> VM-Series
#                (security_vpc) -> GWLBE -> Spoke ALB/NLB -> Spoke VM
#
...
#
#   VPCs: 3 (security + 2 spokes)
#   GWLBs: 1 | VM-Series groups: 1 | TGW route tables: 2
#
# #############################################################################
